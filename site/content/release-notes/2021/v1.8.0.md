...
who is currently processing which shard.
  ...
  Please see [#1433]({{% get-site-data "repositories.core_jvm_repo" %}}/pull/1433) for details.

#### API Changes

- Made `BlackBoxContext` implement `Closeable` (as addition of [#1402]({{% get-site-data "repositories.core_jvm_repo" %}}/pull/1402)).
...
but would NOT inherit `UserId` and `ZoneId`.
  ...
  Check [#1407]({{% get-site-data "repositories.core_jvm_repo" %}}/pull/1407) for details.
...
(see [#1421]({{% get-site-data "repositories.core_jvm_repo" %}}/pull/1421)).

#### Fixes

- Enabled `IntegrationBroker` dispatch events regardless of registration order of subscribing 
and publishing Bounded Contexts (see [#1402]({{% get-site-data "repositories.core_jvm_repo" %}}/pull/1402)).
...
(see [#1405]({{% get-site-data "repositories.core_jvm_repo" %}}/pull/1405)).

- The internals of `IntegrationBroker` were made more thread-safe 
(see [#1423]({{% get-site-data "repositories.core_jvm_repo" %}}/pull/1423)).
...
`AbstractEventReactor` (see [#1423]({{% get-site-data "repositories.core_jvm_repo" %}}/pull/1423)).

#### Performance

- Improved Catch-up caching (see [#1406]({{% get-site-data "repositories.core_jvm_repo" %}}/pull/1406) for details).
...
It supports all the features [recently introduced]({{% get-site-data "repositories.core_jvm_repo" %}}/releases/tag/v1.8.0)
in the corresponding `core-java:1.8.0` release.

### Migration

Due to the changes introduces in [core-java#1433]({{% get-site-data "repositories.core_jvm_repo" %}}/pull/1433),
...
* The reported `DbIterator` issue was resolved. Previously, in some corner cases,
  ...
  See [#155]({{% get-site-data "repositories.jdbc_storage_repo" %}}/pull/155) for more details.
...
In order to support the API change introduced in [core-java#1433]({{% get-site-data "repositories.core_jvm_repo" %}}/pull/1433), 
...
[recently introduced]({{% get-site-data "repositories.core_jvm_repo" %}}/releases/tag/v1.8.0) 
in the corresponding `core-java:1.8.0` release.
